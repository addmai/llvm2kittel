...
InstNamer::InstNamer()
    : m_counter(0), m_bbCounter(0), m_globalCounter(0), m_currFun(NULL),
...
static std::string sanitize(std::string s) {
  std::string res(s);
  res.resize(static_cast<unsigned int>(
      std::remove_if(res.begin(), res.end(), filter) - res.begin()));
  return res;
}
...
void InstNamer::nameBasicBlock(llvm::BasicBlock *BB) {
  if (!m_namedGlobals) {
    ...
    for (llvm::Module::global_iterator global = module->global_begin(),
      ...
      if (llvm::isa<llvm::IntegerType>(
              llvm::cast<llvm::PointerType>(global->getType())
                  ->getContainedType(0))) {
        if (!global->hasName()) {
          std::ostringstream tmp;
  ...
  if (BB->getParent() != m_currFun) {
    ...
    unsigned int c = 0;
    for (llvm::Function::arg_iterator i = m_currFun->arg_begin(),
                                      e = m_currFun->arg_end();
         i != e; ++i) {
      llvm::Argument &arg = *i;
      if (!arg.hasName()) {
